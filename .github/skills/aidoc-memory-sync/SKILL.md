---
name: aidoc-memory-sync
description: 'Use when planning, investigating, or implementing work that may change source code, tests, behavior, APIs, persistence, or output contracts in AI Doc projects. Query relevant AI Doc memory through the local CLI before implementation-oriented source searches or reads, and update memory only after implementation and all relevant validations pass.'
argument-hint: 'Describe the source change to make and the AI Doc project name.'
user-invocable: true
disable-model-invocation: false
---

# AI Doc Memory Synchronization

Keep AI Doc memory aligned with verified source-code changes.
...
2. From the AI Doc repository root, identify the registered project using JSON output:

   ```powershell
   dotnet run --project standalone/src/AiDoc.Cli -- project list --format json
   ```
   ...
   ```powershell
   dotnet run --project standalone/src/AiDoc.Cli -- search "<class, method, feature, or behavior>" --project <project-name> --format json
   ```

   If the target repository is not registered, add it with `project add` only when its path is known and modifying the local AI Doc database is authorized. Use the returned `name` value for every `--project` argument.
...
5. After successful validation, synchronize the change through the CLI:
   ...
   - For an existing memory, append an immutable revision:
     ...
     ```powershell
     dotnet run --project standalone/src/AiDoc.Cli -- memory revise <memory-id> --file <revision.json> --format json
     ```

   - For a newly documented component, create a memory:

     ```powershell
     dotnet run --project standalone/src/AiDoc.Cli -- memory report --project <project-name> --file <memory.json> --format json
     ```
...
7. Verify synchronization by retrieving the current record or history:
   ...
   ```powershell
   dotnet run --project standalone/src/AiDoc.Cli -- memory get <memory-id> --format json
   dotnet run --project standalone/src/AiDoc.Cli -- memory history <memory-id> --format json
   ```
...
- If the CLI cannot build or start, the database cannot be opened, or `AIDOC_HOME` is inaccessible, record the failed preflight and continue with local investigation only when the source task can still be completed safely. Do not treat the failure as an empty memory result.
...
- For tasks that may change source code or tests, always query AI Doc before implementation-oriented source searches, implementation reads, or edits.
- Always update AI Doc after verified source-code changes.
...
- Respect `AIDOC_HOME` when the project uses a non-default local data directory.
