...
using AiDoc.Cli.Commands;

namespace AiDoc.Cli.Tests;

public sealed class CliApplicationTests : IDisposable
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "aidoc-cli-tests", Guid.NewGuid().ToString());
    ...
    public void Run_ReportsAndSearchesMemoryWithJsonContracts()
    {
        ...
        Environment.SetEnvironmentVariable("AIDOC_HOME", Path.Combine(_root, "home"));
    ...
    {
        Directory.CreateDirectory(_root); Environment.SetEnvironmentVariable("AIDOC_HOME", Path.Combine(_root, "home"));
    ...
    public void Run_ReturnsDetailedHelpForProjectAdd()
    {
        ...
        Assert.Equal("aidoc", JsonDocument.Parse(rootHelp.Output).RootElement.GetProperty("data").GetProperty("name").GetString());
    }
    ...
    public void Dispose() { Environment.SetEnvironmentVariable("AIDOC_HOME", null); if (Directory.Exists(_root)) Directory.Delete(_root, recursive: true); }
}