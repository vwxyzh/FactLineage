...
namespace FactLineage.Cli.Commands;

public sealed class CliApplication
{
    private const string HomeEnvironmentVariable = "FACTLINEAGE_HOME";
    private const string LegacyHomeEnvironmentVariable = "AIDOC_HOME";
    private const string DatabaseFileName = "factlineage.db";
    private const string LegacyDatabaseFileName = "aidoc.db";
    ...
    public int Run(string[] args, TextReader input, TextWriter output, TextWriter error)
    {
        ...
        try
        {
            ...
            var storage = GetStoragePaths();
            var database = new SqliteDatabase(storage.DatabasePath);
            database.Migrate();
            var projects = new ProjectService(database);
            var modelStore = new EmbeddingModelStore(Path.Combine(storage.Home, "models", "multilingual-e5-small"));
            ...
            var result = Execute(args, input, projects, memories, new MaintenanceService(database, projects, embeddings), modelStore, storage.Home);
    ...
    private static object Execute(string[] args, TextReader input, ProjectService projects, MemoryService memories, MaintenanceService maintenance, EmbeddingModelStore modelStore, string home)
    {
        ...
        if (args is ["backup", ..]) return new { backupPath = maintenance.Backup(Path.Combine(home, "backups")) };
        ...
        throw new DomainException("INVALID_COMMAND", "Unknown command. Run 'factlineage help' for usage.");
    }

    private static StoragePaths GetStoragePaths()
    {
        if (Environment.GetEnvironmentVariable(HomeEnvironmentVariable) is { Length: > 0 } home)
        {
            return CreateStoragePaths(Path.GetFullPath(home), preferLegacyDatabaseName: false);
        }

        if (Environment.GetEnvironmentVariable(LegacyHomeEnvironmentVariable) is { Length: > 0 } legacyHome)
        {
            return CreateStoragePaths(Path.GetFullPath(legacyHome), preferLegacyDatabaseName: true);
        }

        var localApplicationData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
        var current = CreateStoragePaths(Path.Combine(localApplicationData, "FactLineage"), preferLegacyDatabaseName: false);
        if (File.Exists(current.DatabasePath))
        {
            return current;
        }

        var legacy = CreateStoragePaths(Path.Combine(localApplicationData, "AI Doc"), preferLegacyDatabaseName: true);
        return File.Exists(legacy.DatabasePath) ? legacy : current;
    }

    private static StoragePaths CreateStoragePaths(string home, bool preferLegacyDatabaseName)
    {
        var databasePath = Path.Combine(home, DatabaseFileName);
        var legacyDatabasePath = Path.Combine(home, LegacyDatabaseFileName);
        if (!File.Exists(databasePath) && (preferLegacyDatabaseName || File.Exists(legacyDatabasePath)))
        {
            databasePath = legacyDatabasePath;
        }

        return new StoragePaths(home, databasePath);
    }

    private sealed record StoragePaths(string Home, string DatabasePath);
    ...
    private static object GetHelp(string[] args)
    {
        ...
        return name switch
        {
            "" => new
            {
                ...
                usage = "factlineage <command> [options]",
                commands = new[]
                {
                ...
                examples = new[] { "factlineage help project add", "factlineage search \"login\" --project my-api --format json" }
            },
            "project" => new
            {
                name = "project",
                description = "Manage locally registered projects. Run 'factlineage help project <subcommand>' for details.",
                commands = new[]
                {
            ...
            "project add" => CommandHelp("project add", "Register a local project directory.", "factlineage project add --name <name> --path <path> [--remote-url <url>] [--format <format>]", [], [
                ...
                FormatOption
            ], ["factlineage project add --name my-api --path D:\\code\\my-api --format json"]),
            "project update" => CommandHelp("project update", "Modify a registered project without changing its project ID.", "factlineage project update <name> [--new-name <name>] [--path <path>] [--remote-url <url> | --clear-remote-url] [--format <format>]", [new HelpArgument("<name>", "Existing project name.")], [
            ...
            "project list" => CommandHelp("project list", "List all projects or selected projects in the requested order.", "factlineage project list [--name <name> ...] [--format <format>]", [], [new HelpOption("--name <name>", false, "Repeat to select one or more projects; omit for all projects."), FormatOption], ["factlineage project list --name orders-api --name shared-lib --format json"]),
            "project show" => CommandHelp("project show", "Show one registered project.", "factlineage project show <name> [--format <format>]", [new HelpArgument("<name>", "Existing project name.")], [FormatOption], ["factlineage project show my-api --format json"]),
            "project remove" => CommandHelp("project remove", "Remove a project and all of its memory records without deleting source files.", "factlineage project remove <name> --yes [--format <format>]", [new HelpArgument("<name>", "Existing project name.")], [new HelpOption("--yes", true, "Required confirmation for this destructive operation."), FormatOption], ["factlineage project remove my-api --yes --format json"]),
            "memory" => new { name = "memory", description = "Manage immutable memory records. Run 'factlineage help memory <subcommand>' for details.", commands = new[] { "report", "import", "revise", "get", "export", "history" } },
            "memory report" => CommandHelp("memory report", "Create a memory and version 1 from JSON input.", "factlineage memory report --project <name> (--file <path> | --stdin) [--allow-missing-references] [--format <format>]", [], [new HelpOption("--project <name>", true, "Single target project."), new HelpOption("--file <path>", false, "JSON input file; mutually exclusive with --stdin."), new HelpOption("--stdin", false, "Read JSON input from standard input; mutually exclusive with --file."), new HelpOption("--allow-missing-references", false, "Allow referenced files that do not exist."), FormatOption], ["factlineage memory report --project my-api --file memory.json --format json"], ReportInputSchema),
            "memory import" => CommandHelp("memory import", "Recursively import JSON memory-report files from a directory. Invalid files are reported while other files continue importing.", "factlineage memory import --project <name> --directory <path> [--allow-missing-references] [--format <format>]", [], [new HelpOption("--project <name>", true, "Single target project."), new HelpOption("--directory <path>", true, "Existing directory recursively scanned for .json files."), new HelpOption("--allow-missing-references", false, "Allow referenced files that do not exist."), FormatOption], ["factlineage memory import --project my-api --directory .\\memories --format json"], ReportInputSchema),
            "memory revise" => CommandHelp("memory revise", "Append a new immutable version to an existing memory.", "factlineage memory revise <memory-id> (--file <path> | --stdin) [--allow-missing-references] [--format <format>]", [new HelpArgument("<memory-id>", "Existing memory ID.")], [new HelpOption("--file <path>", false, "JSON input file; mutually exclusive with --stdin."), new HelpOption("--stdin", false, "Read JSON input from standard input; mutually exclusive with --file."), new HelpOption("--allow-missing-references", false, "Allow referenced files that do not exist."), FormatOption], ["factlineage memory revise <memory-id> --file revision.json"], RevisionInputSchema),
            "memory get" => CommandHelp("memory get", "Get a memory and its current version.", "factlineage memory get <memory-id> [--format <format>]", [new HelpArgument("<memory-id>", "Existing memory ID.")], [FormatOption], ["factlineage memory get <memory-id> --format json"]),
            "memory export" => CommandHelp("memory export", "Export a memory's current version as a reusable memory report document.", "factlineage memory export <memory-id> [--format <format>]", [new HelpArgument("<memory-id>", "Existing memory ID.")], [FormatOption], ["factlineage memory export <memory-id> --format json"]),
            "memory history" => CommandHelp("memory history", "List all immutable versions of a memory.", "factlineage memory history <memory-id> [--format <format>]", [new HelpArgument("<memory-id>", "Existing memory ID.")], [FormatOption], ["factlineage memory history <memory-id> --format json"]),
            "search" => CommandHelp("search", "Search memory with hybrid keyword and local semantic retrieval.", "factlineage search <query> (--project <name> ... | --all-projects) [--type <type>] [--limit <count>] [--format <format>]", [new HelpArgument("<query>", "Natural-language or exact code-term query.")], [new HelpOption("--project <name>", false, "Repeat to select one or more projects; mutually exclusive with --all-projects."), new HelpOption("--all-projects", false, "Search every project visible when the command begins; mutually exclusive with --project."), new HelpOption("--type <type>", false, "Filter by feature, api, or decision."), new HelpOption("--limit <count>", false, "Global result limit from 1 through 100; defaults to 10."), FormatOption], ["factlineage search \"login\" --project my-api --format json", "factlineage search \"deprecated\" --all-projects"]),
            ...
            "embedding model download" => CommandHelp("embedding model download", "Download the local ONNX model and SentencePiece tokenizer.", "factlineage embedding model download [--format <format>]", [], [FormatOption], ["factlineage embedding model download --format json"]),
            "embedding backfill" => CommandHelp("embedding backfill", "Generate missing or outdated Embeddings for every memory version in one project.", "factlineage embedding backfill --project <name> [--format <format>]", [], [new HelpOption("--project <name>", true, "Project whose memory versions will be embedded."), FormatOption], ["factlineage embedding backfill --project my-api --format json"]),
            "doctor" => CommandHelp("doctor", "Check database integrity and registered project paths.", "factlineage doctor [--format <format>]", [], [FormatOption], ["factlineage doctor --format json"]),
            "backup" => CommandHelp("backup", "Create a SQLite database backup in the configured data directory.", "factlineage backup [--format <format>]", [], [FormatOption], ["factlineage backup --format json"]),
            "version" => CommandHelp("version", "Show the CLI and output schema versions.", "factlineage version [--format <format>]", [], [FormatOption], ["factlineage version --format json"]),
            _ => throw new DomainException("INVALID_COMMAND", $"Unknown command '{name}'. Run 'factlineage help' for usage.")
        };
    }
